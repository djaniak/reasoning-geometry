...
RMD/ATRMD and the tail aggregation are not new methods. The repository's literature record assigns token-level RMD and trace aggregation to prior work, and shows the tail restriction is required only on Qwen. [PAPER_STRATEGY_RMD.md:28-33] [RELATED_WORK.md:18-51]
