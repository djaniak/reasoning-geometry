...
Scope of the check: the experiment described in
...
| [Fernandez-Boullon & Olivieri, *Patch-Effect Graph Kernels for LLM Interpretability*](https://arxiv.org/abs/2605.06480) (May 2026) **[abstract]** | IOI and variants | GPT-2 small | Activation patching profiles turned into graphs | Model components | Classification accuracy over graph features | No. It explicitly compares against prompt-only and raw patch-effect controls | Prompt-only and raw patch-effect controls | Graph-ML over patching profiles; no task dependency graph, no arithmetic, no written CoT |
...
## 2. Per-family reading: what each family takes and what it leaves
...
off distribution. Neither it nor Heimersheim & Nanda's tutorial raises the
case where the counterfactual value and the literal token at the patched
position
...
verbatim, which is false across most of this literature. And it is a point
about granularity rather than priority: [Shih et
al.](https://arxiv.org/abs/2606.29522) run a computation-versus-copy control,
just not one that scores a literal written
...
replacement-style counterfactual (a different item value, in distribution),
which is exactly what Zhang & Nanda recommend, and this should be stated. And
...
This is the family the review feared, and the concern was justified, but it
takes something different from what was expected. It does not take the step
boundary. It takes
...
[Causal
scrubbing](https://www.greaterwrong.com/posts/JvZhhzycHu2Yd57RN/causal-scrubbing-a-method-for-rigorously-testing)
is the closest prior art for "does the model's computation respect the stated
graph", as §5 of the strategy document anticipated, and reading it closes
rather than opens the concern. Scrubbing takes a hypothesis `h = (G, I, c)`
with `c` an
...
What this family leaves: it has no analogue of the copy competitor, and
because its causal models are small (two boolean variables for Alpaca, a
natural-logic tree for MQNLI) it has no ladder in the number of *written*
steps between the
...
the transformed value. This is a difference in granularity and in what the
task makes possible rather than a gap in the idea of controlling for copying.
...
- [Cywiński et
  ...
  same-intermediate patch as a null, which is structurally our null-flip gate, published
...
back-patching across layers. [Liang & Pan](https://arxiv.org/abs/2602.00449)
come closest to a step-count result, in that CODI stops executing a full
latent rollout at longer hop lengths and concentrates on late intermediates,
but this is on a latent-CoT model with decodability as the outcome.
...
"Extra tokens buy computation independent of their content" is [Pfau et
al.](https://arxiv.org/abs/2404.15758), and this is the result that makes
...
running-state supervision, and the pretrained and final-answer-only controls
stay near baseline, whereas ours is on a pretrained reasoning-tuned checkpoint
with
no such training. Their state is a single order-sensitive bit edited in a
rank-16 subspace at a fixed printed token. They do not score literal reproduction
of that patch-site token as a separate outcome, and no arithmetic is applied to
the edited value. Ours transplants a whole residual state at its native position
and requires the recipient to apply its own remaining arithmetic. The
persistence contrast (their register survives several
...
variable is an assumption the manuscript makes rather than tests. Cite the
paper, state the mitigation, and state the residual. "You measured answer
placement" is the review comment this design most invites, and a claim of
having settled it
invites it twice.
...
formally contradictory, since theirs is about *when* a value is first
available and ours is about *what determines the answer once the value is on
the page*, but a reviewer will read them as contradictory unless the paper
distinguishes them in as many
words.
...
question against us. [Brinkmann et al.](https://arxiv.org/abs/2402.11917)
settle "intermediate results are stored in selected token positions" against
us. That is their abstract's own sentence, and their register-token patching
after layer 4 is
...
gives the mechanism vocabulary, retrieving information when it becomes
necessary, of which D3 is an instance.
...
and [On the Biology of a Large Language
...
recover graph-shaped internal computation from a production model, validate it
by feature intervention, and, in the multi-step reasoning section, demonstrate
the Dallas→Texas→Austin chain by swapping the intermediate and reading the
transformed
output.
...
## 3. Verification status and open questions
...
- **LessWrong's own page truncated.** The direct
  ...
  the [GreaterWrong
  mirror](https://www.greaterwrong.com/posts/JvZhhzycHu2Yd57RN/causal-scrubbing-a-method-for-rigorously-testing)
...
- **[iGSM / Physics of Language Models Part 2.1](https://arxiv.org/abs/2407.20311)
  ...
  only what is unambiguous, namely that iGSM is generated and the model is trained on
  it, and make no claim about whether they run interventions. **Read the SSRN PDF
...
- **A miss, corrected on 2026-08-19.** This pass did not find
  ...
  text and folded into §1a, §2.3 and §2.4. **Read the claim-boundary note
...
- **Searches that returned nothing, as amended.** No paper found that (a) varies
  ...
  **(b) is the amended form.** As originally written, "records a copy
  competitor alongside a directional patching outcome", it was incorrect: Shih
  et al.'s conflicting-continuation control is exactly that, in the form of a
  contrast
  between two rule-defined branches. Absence of evidence is weak evidence
  here, since (c) and (d) are the kind of control that is usually reported in
  an appendix, and the Shih omission shows that the search vocabulary was
...
- **Not searched, and worth a pass before submission:** the state-tracking
  ...
  register", "state tracking", "working memory", "transition system") should
  be run against the same questions §2.3 asks, since the search terms used
  here were
