...
*Tables are auto-generated by `summarize.py` from per-stage result JSONs. The
current-evidence note below is manually maintained and was last reconciled with
the artifacts on 2026-07-11.*

## Current Evidence Status

The safest current conclusion is narrower than several historical tables below:

> On Qwen MATH-500, global RMD primarily captures between-prompt solvability.
> A leave-one-trace-out prompt-local background does not recover useful
> same-prompt correctness geometry. The old DeepSeek within-prompt result is
> dominated by truncated, unparsed generations and is not evidence of
> trace-level correctness.

Latest Qwen `500 x 8` out-of-fold results:

| Result | L7 | L14 | L21 | Interpretation |
|:---|---:|---:|---:|:---|
| Global RMD pooled AUC | 0.736 | 0.763 | 0.786 | Strong across prompts |
| Global RMD within-prompt pair AUC | 0.551 | 0.550 | 0.602 | Modest before parseability filtering |
| Prompt-local RMD pooled AUC | 0.379 | 0.402 | 0.320 | Anti-predictive globally |
| Prompt-local RMD within-prompt pair AUC | 0.499 | 0.480 | 0.529 | Approximately chance |
| Prompt-local RMD top-1 Pass@1 | 0.558 | 0.532 | 0.548 | No gain over random 0.557; below majority 0.596 |

The primary honest contrast is RMD against trace length. At L21, RMD exceeds
length by `+0.055` pooled AUC (95% prompt-bootstrap CI `[+0.021, +0.093]`) and
by `+0.116` within-prompt macro AUC (`[+0.065, +0.171]`) on all traces. After
restricting to parseable Qwen traces, however, L21 within-prompt RMD is `0.574`,
prompt-local RMD is `0.483`, and log-probability is `0.649`. Thus the current
data do not support full-trace RMD as a competitive per-attempt reranker.

DeepSeek's historical `~0.93` within-prompt AUC must not be quoted as a
correctness result: `45.4%` of its old Best-of-N traces were unparsed and almost
all of those hit the 2048-token cap. Parseable-only evaluation leaves only 13
mixed prompts. Budget probes found `12.5%` cap/unparsed at 8192 tokens for
DeepSeek-R1-Distill-Qwen-7B (`n=24`) and `0%` at 12288 for
DeepSeek-R1-Distill-Llama-8B (`n=24`). These small probes size future collection;
they do not repair the historical artifacts.

Claims currently supported:

- Relative/background-subtracted geometry is materially better than raw
  target-only Mahalanobis distance.
- Qwen RMD contains a robust between-prompt solvability component beyond length
  at the deepest tested sparse layer.
- Global and prompt-local full-trace geometry do not provide a reliable Qwen
  Best-of-N selector; majority vote and log-probability remain stronger.

Claims rejected or not yet established:

- Rejected: DeepSeek `~0.93` within-prompt AUC measures reasoning correctness.
- Rejected on Qwen: a simple prompt-local RMD background reveals a hidden
  same-prompt correctness manifold.
- Not established: distillation causally creates low-dimensional correctness
  geometry; existing checkpoint lineages and truncation differ.
- Not established: parseable-only RMD selective prediction generalizes across
  models with uncertainty intervals against length and trained probes.
...
| qwen | math500 | L7 | entropy | 0.572 | 0.500 | 0.500 | 1.000 | 0.139 |
| qwen | math500 | L7 | logprob | 0.575 | 0.559 | 0.589 | 0.669 | 0.153 |
| qwen | math500 | L7 | length | 0.731 | 0.500 | 0.500 | 1.000 | 0.452 |
...
| qwen | math500 | L14 | entropy | 0.572 | 0.500 | 0.500 | 1.000 | 0.139 |
| qwen | math500 | L14 | logprob | 0.575 | 0.559 | 0.589 | 0.669 | 0.153 |
| qwen | math500 | L14 | length | 0.731 | 0.500 | 0.500 | 1.000 | 0.452 |
...
| qwen | math500 | L21 | entropy | 0.572 | 0.500 | 0.500 | 1.000 | 0.139 |
| qwen | math500 | L21 | logprob | 0.575 | 0.559 | 0.589 | 0.669 | 0.153 |
| qwen | math500 | L21 | length | 0.731 | 0.500 | 0.500 | 1.000 | 0.452 |
...
| qwen | math500 | L7 | top1_entropy | 0.574 |
| qwen | math500 | L7 | top1_logprob | 0.574 |
| qwen | math500 | L7 | top1_length | 0.574 |
...
| qwen | math500 | L14 | top1_entropy | 0.574 |
| qwen | math500 | L14 | top1_logprob | 0.574 |
| qwen | math500 | L14 | top1_length | 0.574 |
...
| qwen | math500 | L21 | top1_entropy | 0.574 |
| qwen | math500 | L21 | top1_logprob | 0.574 |
| qwen | math500 | L21 | top1_length | 0.574 |
