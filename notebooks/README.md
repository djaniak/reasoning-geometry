...
| [14_rmd_workshop_story](14_rmd_workshop_story.ipynb) | Between-prompt | **Start here for the RMD paper.** The narrative spine of the selective-prediction half: the pooled and within-prompt estimands, the outcome protocol, the increment that survives it, the ATRMD-versus-tail scope result, and the peer cost ladder with uncertainty. Reads every number out of a committed artifact; the full-refit gate in §4 is still open. Notebook 17 is its evidence trail. |
