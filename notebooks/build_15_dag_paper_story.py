...
this notebook fails visibly rather than going stale without warning.*
...
read as an explanation, and an explanation is only worth reading if the steps in it actually determine the result.
...
and known, three different digits are named in advance for every item: the clean answer, the value *implied* by carrying the donor's number through the
...
1. **A patched value is mostly propagated, and sometimes copied.** When the
   answer reads the patched node directly, the readout moves to the *implied* digit (24/24 pre-registered items, 267/315 (84.8%) across the 48-item campaign), while matched non-ancestor, null and surface edits leave the answer where it was (0/192 control rows moved, at any layer). The remaining 46 land
...
2. **The channel reaches exactly one step, and the written intermediate is what
   ...
   patched node and the answer and the same edit at the same layer moves nothing: 0/24 pre-registered, 0/432 across every site two or more steps out,
   ...
   step is. The contrast is not the difficulty or the token distance that travel with depth: under a pre-registered match on clean confidence (to 0.0007) and
...
3. **Three named digits and a re-scorable verdict are what make 1 and 2
   falsifiable.** The verdict space is three-valued (*invalid test* / *positive* / *scientific negative*) and is a policy over stored rows, never a hand edit,
   ...
   The rule is reported and left alone (§10), because rewriting a quorum after seeing which arms it fails is the retroactive move the verdict design exists to
   prevent.
...
**The clean run.** The model is shown a finished arithmetic trace, one line per step, `name = lhs op rhs = value # tag`, and we read the next-token distribution
...
**The patched run.** The model reads the *clean* text again, and part-way through the forward pass, at layer 13 of 28, we overwrite what it is holding at those
...
every item. That is what turns one undifferentiated outcome, "the answer changed", into three that can be told apart.

| word | what it means here |
|:---|:---|
| **node** | one line of the trace. In `c = a - 1 = 6 # t`, the node is `c`. |
| **target** | the node whose stated digit we read: the answer. |
| **ancestor** | a node the target's value depends on, and the one worth patching. |
...
things that travel with depth (§7), whose one structural difference is the blue line, an intermediate result the trace states, and that the patch leaves clean.
The bar chart is that one item's readout before and after: the clean digit is
emptied and the mass lands on `implied`, the value the donor's number takes after the recipient's own remaining arithmetic, rather than on `raw`, the digit the donor literally wrote at the patched position. That distinction is the whole reason the
...
thought, and the numbers must not be read as "influence decays with distance"; the `gap` control in §5 is what separates the two.
...
Positioning against the interpretability literature is not written yet; `RELATED_WORK.md` covers the project's other half.
""")
...
three seeds, and the two places where the small-N picture was too clean are both there: the raw digit is not 0%, and one site here was never patched at all.*
""")
...
disturb. Second, the depth-2 and depth-3 arms are **scientific negatives** rather than failed tests: the machinery ran, the controls behaved, and the effect was absent.
""")
...
`gap` control puts decoy lines between the chain and the target, raising token distance without adding a step to the path, using the same five spines at three placements.
...
compute the intermediate latently". What the arms support is that **no behaviourally usable carried intermediate was detected**, and a behavioural failure after removing a written value cannot separate *computing* it from
...
it. If the answer is fixed by that written line, an edit landing on it should carry, and §9 is that experiment. It does, 288/288. That does not rescue the
...
Depth does not travel alone. In the archived runs the eligible clean `p(target)` supports do not even touch (0.666–0.961 at depth 1 against 0.966–0.999 at depth 2), so every depth-1 success is on an item the model was unsure of and every
...
depth-1 range: the matched test is run on the high-confidence half of where the original depth-1 effect was found, which is the harder half for claim 1, and the reason a
...
every item was re-checked against its stage-A measurement (ancestor distance, target value, gap, and the clean readout to 1e-6) before being patched.
""")
...
weakens at 20 and is gone at 27. Layer 13 is inherited from Table 1 rather than re-searched, so this run is confirmatory for the *contrast*, not for the layer.
Middle: at depth 1 the clean answer is emptied and the implied digit takes almost all of it, but the raw digit is promoted about 200-fold (0.0005 → 0.107) even
though it never wins, so "the recipient transforms the donor value" is a cleaner
sentence than the measurement deserves. Right: at depth 2 the patch is **not inert, it is unaimed**: its ancestor rows move the readout about twelve times as
...
own token positions, so a depth-2 item now carries two edits against one clean readout: its ancestor, two steps from the answer, and its written intermediate,
...
between it and the answer. This *does* decay, over three orders of magnitude (0.99 at one step, 0.035–0.090 at two, 0.003 at three), and a paper reporting only this would say influence fades with depth, and would have the graded curve to
show for it. Middle, on a linear axis: the same sites, scored by which digit ends
up on top. There is no fade. One step works, two steps do not, and the two-step sites are not weakly wrong; they are exactly as clean as no patch at all
...
token distance instead. The curves never approach each other: between 16 and 30 tokens from the answer, one-step sites land 97/113 and two-step sites 0/217, at
...
table(PAIRED, "Table 11 · the two edits inside one item, layer 13",
      ...
      **{"exact sign test": "{:.1e}"},
      note=("The cleanest form of the depth result, because nothing is matched "
            ...
            "two steps out, and it is as inert as the ancestor beside it: "
...
with the ancestor pushed about 13 tokens further out each time, so pooling them counts one item three times. Paired properly they say something §5 could not, and
...
md(r"""
This is the third time one manipulation has arrived bundled with clean difficulty: §7 for depth, §9's own step ladder, and now the gap. It is worth
...
**The verdict policy does not survive its own N.** The arm scorer asks the surface control to pass in `max(1, n − 1)` items, i.e. "all but one". At the n = 5
...
the answer installs the value implied by the recipient's own chain (555/603 sites across the campaign, 24/24 under pre-registered matching), and a patch two or more steps out does not, 0/432, leaving the clean answer intact in all of
...
alive when it sits one step out (288/288), in the same items. The contrast is not explained by clean confidence or token distance: matched to 0.0007 and 1 token in §8, and split 97/113 against 0/217 inside a single distance band in §9. What
is carried is the donor's *stated* result rather than a recomputation from its
operands (§4, pilot strength). The controls are quiet wherever the clean readout is stable: non-ancestor, null and surface-null rows leave the answer alone in
...
**Does not.** It does not show *why* a second step blocks the patch (§6); the omission arm that would answer it destroys the clean behaviour it needs. It says
nothing about MATH-500, about natural traces, or about the 7–8B models the rest of the project measures: the family is synthetic, the values are single digits,
...
which a mutually donatable batch cannot contain; the registration named it and stage B could not run it, so that is a protocol deviation, and it is the control
...
three archived arms predate the `depth` / `gap` / distance fields entirely; those were recovered by regenerating the items and accepted only because the
...
def what(family):
    # A new results directory should stop this table rather than be given a label
    # automatically.
...
Each results directory carries a `README.md` with the command as it was run, the run commit and what the arm is for; those predate the move of these scripts under `dag/`, and the flags are unchanged. `dag/dag_pooling.py` builds
